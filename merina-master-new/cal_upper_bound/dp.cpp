...
unsigned int TOTAL_VIDEO_CHUNCK = 150;
...
static const int VIDEO_BIT_RATE[] = {145, 365 ,730, 1100, 2000, 3000, 4500, 6000, 7800, 10000};  // Kbps
unsigned int DEFAULT_QUALITY = 1;
unsigned int MAX_QUALITY = 9;
float REBUF_PENALTY = 6;// 1 sec rebuffering -> 3 Mbps
float REBUF_PENALTY_LOG = 4.23;
int SMOOTH_PENALTY = 1;
float INVALID_DOWNLOAD_TIME = -1;
std::string COOKED_TRACE_FOLDER = "/home/ubuntu/Whr/merina-master-new/envs/traces/test_file/";
std::string OUTPUT_FILE_PATH = "/home/ubuntu/Whr/merina-master-new/cal_upper_bound/results_up_bound/";
std::string RESULTS_FILE_NAME = "log_sim_dp";
...
std::vector<std::vector<unsigned int> > get_video_sizes() {
	...
	for(int bit_rate = 0; bit_rate < BITRATE_LEVELS; bit_rate ++) {
        ...
        video_size.push_back(static_cast<int>(VIDEO_BIT_RATE[bit_rate] * 250));
...
int main(int argc, char *argv[]) {
    ...
    if(arg2 == "log"){
        output_path = OUTPUT_FILE_PATH + "log/" + arg1 +"/";
        int status = mkdir((output_path ).c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
    }
    else if(arg2 == "lin"){
        output_path = OUTPUT_FILE_PATH + "lin/" + arg1 +"/";
        int status = mkdir((output_path ).c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
    }
    else {
    printf("第二个参数错误!!!!!\n\n\n\n");
