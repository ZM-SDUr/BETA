...
TEST_FILE_Newfile = '../Results_Newfile_CUHK_11bit/test/lin/Newfile_CUHK/'
TEST_FILE_Newfile_NEXT = '../Results_Newfile_next/test/lin/Newfile_CUHK/'
...
TEST_LOG_FILE_Newfile_LOG_NEXT = '../Results_Newfile_next/test/log/Newfile_CUHK/'
...
def main():
	...
	log_files = os.listdir(results_folder1)
	for log_file in log_files:
		...
		with open(results_folder1 + log_file, 'rb') as f:
			for line in f:
				parse = line.split()
				if len(parse) <= 1:
					break
	...
	log_files = os.listdir(results_folder2)
	for log_file in log_files:
		...
		with open(results_folder2 + log_file, 'rb') as f:
			for line in f:
				parse = line.split()
				if len(parse) <= 1:
					break
	...
	for l in time_all[schemes_show[0]]:
		...
		if schemes_check:
			...
			for scheme in schemes_show:
				...
				temp = np.mean(raw_reward_all[scheme][l][INIT_CHUNK:VIDEO_LEN])
				if scheme == "test_a3c" and temp<0:
					print(l)
					print(temp)
				reward_all[scheme].append(temp)
	...
	legend = ax.legend(handles = ax.lines,labels = SCHEMES_REW, fontsize = 18, )
	...
	legend = ax.legend(handles = ax.lines,labels = comparison_schemes_names, loc='best', fontsize = 18)
