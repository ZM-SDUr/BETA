...
def evaluation(actor_net, vae_net, log_path_ini, test_env, \
    s_info, s_len, c_len, total_chunk_num, \
        bitrate_versions, rebuffer_penalty, smooth_penalty, a_dim, args, summary_dir_, add_str):
    ...
    if summary_dir_!= '':
        ...
        for test_log_file in test_log_files:
            if add_str in test_log_file:
                os.remove(test_log_folder + test_log_file)
    ...
    for video_count in tqdm(range(len(all_file_name))):
        while True:
            ...
            if args.stocha:
                ...
                # greedy policy
                selection = int(torch.argmax(prob).squeeze().cpu().numpy())
                bit_rate = calculate_from_selection(selection, last_bit_rate)
