...
class Actor(torch.nn.Module):
    def __init__(self, a_dim, latent_dim, state_channels, his_dim, hidden_layers = None):
        ...
        in_channels_ = self.latent_mlp_output + 5 * self.FE_cnn_channels + 5 * self.FE_output
    ...
    def forward(self, states_input, latent_input):
        ...
        # video chunk sizes
        #下一个块大小
        fut_inputs_1_ = states_input[:, 4:12, -1]
        ...
        # past download time 
        download_time_ = states_input[:, 12:13, :]
        download_time_ = download_time_.view(-1, self.num_flat_features(download_time_))

        ## process feature exactor 
        # video chunk sizes 128*5
        #FeatureExactor_CNN: 处理未来的块大小的输入 (fut_inputs_1_)。
        ...
        #FeatureExactor_latent: 处理潜在输入 (latent_input)
        ...
        #FeatureExactor_MLP 处理缓冲区大小、上一个块的比特率、剩余块的数量
        ...
        ## Aggregation
        #将所有特征提取器的输出合并（torch.cat）成一个单一的张量，这个张量包含了从输入状态和潜在信息中提取出来的所有特征。并且作为隐藏层的输入
