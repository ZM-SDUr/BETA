...
from algos.train_im_v4_10bit import train_iml_v4
from algos.test_v5_10bit import test
from algos.train_ppo_v6_10bit import train_ppo_v6
import envs.env_log_10bit as env
import envs.fixed_env_log_10bit as env_test
...
TOTAL_CHUNK_NUM = 48
REBUF_PENALTY_lin = 50
REBUF_PENALTY_log = 50
SMOOTH_PENALTY = 1
...
TRAIN_TRACES = './envs/traces/L1-5/'
#VALID_TRACES = './envs/traces/fcc_and_hsdpa/test_traces/'
VALID_TRACES = './envs/traces/merina_test_trace/'

ADP_TRAIN_TRACES = './envs/traces/puffer_211017/cooked_traces/'
ADP_VALID_TRACES = './envs/traces/puffer_211017/test_traces/'

SUMMARY_DIR = './Results_10bit_ppo/sim'
#SUMMARY_DIR = './Results_Newfile_CUHK/sim'
MODEL_DIR = './models'

#lin 
TEST_MODEL_ACT_LIN = './Results_10bit/sim/lin/merina/policy_merina_7500.model'
TEST_MODEL_VAE_LIN = './Results_10bit/sim/lin/merina/VAE_merina_7500.model'

TEST_MODEL_ACT_LOG = './Results_10bit/sim/log/merina/policy_merina_15400.model'
TEST_MODEL_VAE_LOG = './Results_10bit/sim/log/merina/VAE_merina_15400.model'
...
parser.add_argument('--batch-size', nargs='?', const=64, default=64, type=int, help='Minibatch size for training')  #训练的宽度
parser.add_argument('--ppo-ups', nargs='?', const=2, default=2, type=int, help='Update numbers in each epoch for PPO')  #每次训练是模型更新的次数
parser.add_argument('--explo-num', nargs='?', const=32, default=32, type=int, help='Exploration steps for roll-out')      #训练时，向重缓冲去存入的信息轮次
parser.add_argument('--ro-len', nargs='?', const=25, default=25, type=int, help='Length of roll-out')  #训练时，每轮向重缓冲去存入的信息个数
...
def main():
    ...
    if args.tf:
        ...
        log_save_dir = TEST_LOG_FILE_Newfile_LOG if args.log else TEST_LOG_FILE_Newfile
    ...
    if args.test:
        ...
        if args.log:
            log_save_dir = TEST_LOG_FILE_Newfile_LOG +"/"
        else:
            log_save_dir = TEST_LOG_FILE_Newfile + "/"
        ...
        if args.log:
            log_dir_path= SUMMARY_DIR+'/log/'
        else:
            log_dir_path = SUMMARY_DIR + '/lin/'
        ...
        train_env = env.Environment(all_cooked_time=all_cooked_time, all_cooked_bw=all_cooked_bw, all_file_name = all_file_names,video_size_file= video_size_file)
        ...
        if args.adp:
            ...
            model_save_dir = MODEL_DIR + '/' + add_str + '/8bitrate' + '/'
            if not os.path.exists(model_save_dir):
                os.mkdir(model_save_dir)
            model_actor_para, model_vae_para = train_iml_v4(IMITATION_TRAIN_EPOCH, train_env, valid_env, args, add_str, log_dir_path,device)
            print(33333)
