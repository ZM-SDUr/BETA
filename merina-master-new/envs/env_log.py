...
class Environment:
    def __init__(self, all_cooked_time, all_cooked_bw,all_file_name, video_size_file, random_seed=RANDOM_SEED):
        assert len(all_cooked_time) == len(all_cooked_bw)
        assert len(all_cooked_time) == len(all_file_name)
    ...
    def get_video_chunk(self, quality):
        ...
        #     print(self.all_file_name[self.trace_idx])
        # print(self.all_file_name[self.trace_idx])
        ...
        if self.video_chunk_counter >= self.total_chunk_num:
            ...
            if self.trace_idx >= len(self.all_cooked_time):
                self.trace_idx = 0
            print("新跟踪文件名：" + self.all_file_name[self.trace_idx])
            self.cooked_time = self.all_cooked_time[self.trace_idx]
            self.cooked_bw = self.all_cooked_bw[self.trace_idx]
