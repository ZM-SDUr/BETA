...
#VIDEO_BIT_RATE = [1000 , 2000, 5000 , 8000 , 12000 , 16000 , 20000, 25000 , 30000 ,35000,  40000 , 50000]
...
RANDOM_SEED = 42
RAND_RANGE = 1000
SUMMARY_DIR = './results/results5'
LOG_FILE = './results/results5/log'
TEST_LOG_FOLDER = './test_results/'
TRAIN_TRACES = '../L1-5/'
NN_MODEL = './results/results4/nn_model_ep_17000.ckpt'
#NN_MODEL = None

def testing(epoch, nn_model, log_file):
    # clean up the test results5 folder
    os.system('rm -r ' + TEST_LOG_FOLDER)
    if not os.path.exists(TEST_LOG_FOLDER):
        os.makedirs(TEST_LOG_FOLDER)
    # run test script
    os.system('/home/ubuntu/miniconda3/bin/conda run -n tf-wzm rl_test.py ' + nn_model)

    # 使用 subprocess.run，这里 args 是一个列表，每个元素是命令的一部分，这有助于正确处理空格和特殊字符
    result = subprocess.run([
        '/home/ubuntu/miniconda3/bin/conda', 'run', '-n', 'tf-wzm', 'python','rl_test.py', nn_model
...
def central_agent(net_params_queues, exp_queues):
    ...
    with tf.compat.v1.Session() as sess, open(LOG_FILE + '_test', 'w') as test_log_file:
        ...
        TRAIN_SUMMARY_DIR = './results5/' + curr_time + '/train'
        TEST_SUMMARY_DIR = './results5/' + curr_time + '/test'
...
def agent(agent_id, all_cooked_time, all_cooked_bw, net_params_queue, exp_queue):
    ...
    #os.environ['CUDA_VISIBLE_DEVICES'] = str(agent_id%6)
    ...
    with tf.compat.v1.Session() as sess, open(LOG_FILE + '_agent_' + str(agent_id), 'w') as log_file:
        ...
        while True:  # experience video streaming forever
            ...
            reward = VIDEO_BIT_RATE[bit_rate]/1000.\
                     - REBUF_PENALTY * rebuf \
                     - SMOOTH_PENALTY * np.abs(VIDEO_BIT_RATE[bit_rate]-VIDEO_BIT_RATE[last_bit_rate])/1000.
            ...
            #          - REBUF_PENALTY * rebuf \
            #          - SMOOTH_PENALTY * np.abs(HD[bit_rate] - HD[last_bit_rate])
