...
def f():
    ...
    all_cooked_time, all_cooked_bw, all_file_names = load_trace.load_trace('../BETA2/test-L14/')
    ...
    Model_reward_list = []
    get_model1 = tf.keras.models.load_model(gb.MODEL_SAVED+'actor_model' + str(model_num))
    get_model2 = tf.keras.models.load_model(gb.MODEL_SAVED + 'actor_model' + str(model_num-1))
    class_model = tf.keras.models.load_model('../../calculate_opt/MODELS-th'+str(700)+'/class_model300')
    trace_test_count = 0
    DEFAULT_BITRATE = 200
    ...
    while True:
        ...
        data = all_cooked_bw[15][:300]
        # 转换数据格式以适应模型输入（假设模型需要的输入格式是[1, 300]）
        data = tf.expand_dims(data, axis=0)
        # 使用模型进行预测
        #prediction = class_model.predict(data)

        # 选择具有最高概率的类别
        #tag = tf.argmax(prediction, axis=1).numpy()[0]
        tag = 1
        if tag == 1:
            Action_model = tf.squeeze(get_model1(pre_state))
        else:
            Action_model = tf.squeeze(get_model2(pre_state))
...
'''
            log_path = "../BETA2/test_all-HD/" + all_file_names[net_env.trace_idx - 1]
