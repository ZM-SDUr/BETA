...
LOG_FILE = './log_file_4G-linear/'
#TEST_LOG_FOLDER = '../test_log_file/'
MODEL_SAVED = './model-linear/'
...
if new_model == 1:
    ...
    actor_model = tf.keras.models.load_model('./model-L15/actor_model'+str(Model_num))
    critic_model1 = tf.keras.models.load_model('./model-L15/critic1_model'+str(Model_num))
    critic_model2 = tf.keras.models.load_model('./model-L15/critic2_model'+str(Model_num))

    target_actor = tf.keras.models.load_model('./model-L15/target_actor'+str(Model_num))
    target_critic1 = tf.keras.models.load_model('./model-L15/target1_critic'+str(Model_num))
    target_critic2 = tf.keras.models.load_model('./model-L15/target2_critic'+str(Model_num))

buffer = bf.Buffer(100000, 64)
