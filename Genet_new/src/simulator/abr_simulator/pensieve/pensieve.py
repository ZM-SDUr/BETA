...
from simulator.abr_simulator.utils import plot_abr_log, linear_reward, hd_reward,log_reward
...
#  export PYTHONPATH=/home/ubuntu/ZimingWang/N_step_TD3_Remote/Genet_new/src:$PYTHONPATH
...
class Pensieve(BaseAbr):
    ...
    def __init__(self, model_path: str = "", s_info: int = 6, s_len: int = 8,
        ...
        elif self.s_info == 6 and self.s_len == 8 and self.a_dim == 8:
    ...
    def _test(self, actor: a3c.ActorNetwork, trace: AbrTrace,
        ...
        while True:  # serve video forever
            ...
            if is_log:
                reward = log_reward(bit_rate,last_bit_rate, rebuf)
            else:
            ...
            state[4, : BITRATE_DIM] = (np.array(next_video_chunk_sizes) / M_IN_K / M_IN_K
            )  # mega byte
    ...
    def test(self, trace: AbrTrace, video_size_file_dir: str, save_dir: str,is_log: bool):
        assert not self.train_mode
        return self._test(self.actor, trace, video_size_file_dir, save_dir,is_log)

    def test_on_traces(self, traces: List[AbrTrace], video_size_file_dir: str,
                       save_dirs: List[str],is_log: bool):
        assert not self.train_mode
        rewards = []
        for trace, save_dir in zip(traces, save_dirs):
            rewards.append(self.test(trace, video_size_file_dir, save_dir,is_log))
        return rewards

    def train(self, trace_scheduler, val_traces: List[AbrTrace],
              save_dir: str, num_agents: int, total_epoch: int,
              video_size_file_dir: str,is_log: bool, model_save_interval: int = 1000 ):
        ...
        for i in range(num_agents):
            agents.append(mp.Process(
                ...
                args=(TRAIN_SEQ_LEN, self.s_info, self.s_len, self.a_dim,
                      ...
                      video_size_file_dir, self.jump_action,is_log)))
        for i in range(num_agents):
            agents[i].start()
...
def agent(train_seq_len: int, s_info: int, s_len: int, a_dim: int,
    ...
    #for i in range(len(net_env.trace.bandwidths)):
        #print(net_env.trace.timestamps[i], net_env.trace.bandwidths[i])
