...
def parse_args():
    ...
    parser.add_argument("--bo-rounds", type=int, default=11,
                        help="Rounds of BO.")
...
def black_box_function(min_bw, max_bw, bw_change_interval, link_rtt,
                       ...
                       video_size_file_dir, jump_action, is_log, save_dir="" ):
    '''
    ...
    if jump_action:
        pensieve = Pensieve(model_path, 6, 8, 8)
    else:
        pensieve = Pensieve(model_path)
    rlrewards = pensieve.test_on_traces(traces, video_size_file_dir, save_dirs, is_log)
    gap = np.mean(hrewards) - np.mean(rlrewards)
    tf.reset_default_graph()  # Çå³ýÄ¬ÈÏÍ¼
    return gap
...
class Genet:
    ...
    def __init__(self, config_file: str, save_dir: str,
                 ...
                 model_path: str, video_size_file_dir: str, train_trace_dir: str,real_trace_prob: float,nagent: int = 10,
                 n_init_pts: int = 10, n_iter: int = 5, seed: int = 42,
                 jump_action: bool = False,is_log: bool = False):
        ...
        self.is_log = is_log
        self.train_trace_dir=train_trace_dir
    ...
    def train(self, rounds: int, epoch_per_round: int, val_dir: str,
        ...
        for i in range(rounds):
            ...
            optimizer = BayesianOptimization(
                    ...
                    save_dir=os.path.join(training_save_dir, 'bo_traces'),is_log =self.is_log ),
            ...
            cmd = "python /home/ubuntu/ZimingWang/N_step_TD3_Remote/Genet_new/src/simulator/abr_simulator/pensieve/train.py " \
            ...
            cmd += "udr " \
                    ...
                    "--train-trace-dir={train_trace_dir}".format(
                        ...
                        train_trace_dir=self.train_trace_dir)
            print(cmd)
...
def main():
    ...
    if args.heuristic == 'mpc':
        heuristic = RobustMPC(is_log = args.log)
    ...
    genet = Genet(args.config_file, args.save_dir, black_box_function,
                  ...
                  train_trace_dir=args.train_trace_dir, real_trace_prob=args.real_trace_prob,
                  nagent=args.nagent, seed=args.seed,
                  jump_action=args.jump_action,is_log=args.log)
    genet.train(args.bo_rounds, epoch_per_round=10000, val_dir=args.val_trace_dir)
