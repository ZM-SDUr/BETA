...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, bitrate_dim):
        ...
        # 这个梯度将由评论家网络提供
        self.act_grad_weights = tf.compat.v1.placeholder(tf.float32, [None, 1])    # 动作梯度权重的占位符

        self.entropy_weight = tf.compat.v1.placeholder(tf.float32)       # 熵权重的占位符
        self.lr_rate = tf.compat.v1.placeholder(tf.float32)              # 学习率的占位符
        ...
        # Combine the gradients here  在这里合并梯度
        self.actor_gradients = tf.gradients(self.obj, self.network_params)    # 计算目标函数关于网络参数的梯度

        # Optimization Op
        self.optimize = tf.compat.v1.train.RMSPropOptimizer(self.lr_rate).\
            apply_gradients(zip(self.actor_gradients, self.network_params))   # 使用RMSProp优化器应用梯度更新
