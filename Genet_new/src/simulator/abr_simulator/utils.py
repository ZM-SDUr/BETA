...
from simulator.abr_simulator.constants import (HD_REWARD, M_IN_K, REBUF_PENALTY,REBUF_penalty,
...
def linear_reward(current_bitrate: int, last_bitrate: int, rebuffer: float):
    ...
    """
    reward = VIDEO_BIT_RATE[current_bitrate] / M_IN_K - REBUF_PENALTY * rebuffer - \
        SMOOTH_PENALTY * np.abs(VIDEO_BIT_RATE[current_bitrate] - VIDEO_BIT_RATE[last_bitrate]) / M_IN_K
    return reward

def log_reward(current_bitrate: int, last_bitrate: int, rebuffer: float):
    """ Return linear QoE metric.
    Args
        current_bitrate: current bit rate (kbps).
        last_bitrate: previous bit rate (kbps).
