...
from simulator.abr_simulator.utils import plot_abr_log, linear_reward,log_reward
...
for combo in itertools.product([0, 1, 2, 3, 4, 5, 6, 7], repeat=MPC_FUTURE_CHUNK_COUNT):
...
def calculate_rebuffer(size_video_array, future_chunk_cnt, buffer_size,
                       bit_rate, last_index, future_bandwidth, bitrate_options,is_log,
    ...
    for full_combo in action_combos:
        ...
        for position in range( 0, len( combo ) ):
            ...
            if is_log:
                reward = log_reward(chunk_quality, last_quality, curr_rebuffer_time)
            else:
                reward = linear_reward(chunk_quality, last_quality, curr_rebuffer_time)
        ...
        if reward >= max_reward:
            ...
            max_reward = reward
            send_data = 0
            if len(best_combo) != 0:  # some combo was good
...
def next_possible_bitrates(br):
    ...
    next_brs = [a for a in next_brs if 0 <= a <= 9]
    return next_brs
...
class RobustMPC(BaseAbr):
    ...
    def __init__(self, is_log: bool , jump_action_flag: bool = False, plot_flag: bool = False):
    ...
    def get_next_bitrate(self, size_video_array, future_chunk_cnt, buffer_size,
        ...
        return calculate_rebuffer(
            ...
            last_index, self.future_bandwidth, np.array(VIDEO_BIT_RATE),self.is_log,
